@echo off
rem -------------------------------------------------------------------------
rem Swarm startup Script for Windows
rem -------------------------------------------------------------------------
...
pushd %DIRNAME%..
set "RESOLVED_SWARM_HOME=%CD%"
popd
set UNQUOTED_SWARM_HOME=%SWARM_HOME:"=%
rem attempt to unquote again to remove quote if envvar was not set
set UNQUOTED_SWARM_HOME=%UNQUOTED_SWARM_HOME:"=%
set QUOTED_SWARM_HOME="%UNQUOTED_SWARM_HOME%"
rem should only a = if envvar was not set
if "%UNQUOTED_SWARM_HOME%" == "=" (
  set "UNQUOTED_SWARM_HOME=%RESOLVED_SWARM_HOME%"
  set QUOTED_SWARM_HOME="%RESOLVED_SWARM_HOME%"
)
pushd %QUOTED_SWARM_HOME%
set "SANITIZED_SWARM_HOME=%CD%"
popd

if /i "%RESOLVED_SWARM_HOME%" NEQ "%SANITIZED_SWARM_HOME%" (
   echo.
   echo   WARNING:  SWARM_HOME may be pointing to a different installation - unpredictable results may occur.
   echo.
   echo       SWARM_HOME: %QUOTED_SWARM_HOME%
   echo.
)
...
if exist "%UNQUOTED_SWARM_HOME%\jboss-modules.jar" (
    set RUNJAR="%UNQUOTED_SWARM_HOME%\jboss-modules.jar"
) else (
  echo Could not locate "%UNQUOTED_SWARM_HOME%\jboss-modules.jar".
...
if "x%SWARM_MODULEPATH%" == "x" (
  set  "SWARM_MODULEPATH=%UNQUOTED_SWARM_HOME%\modules"
)

rem Set the deployment base dir
if "x%SWARM_BASE_DIR%" == "x" (
  set  "SWARM_BASE_DIR=%UNQUOTED_SWARM_HOME%\deploy"
)
rem Set the standalone log dir
if "x%SWARM_LOG_DIR%" == "x" (
  set  "SWARM_LOG_DIR=%UNQUOTED_SWARM_HOME%\log"
)
rem Set the standalone configuration dir
if "x%SWARM_CONFIG_DIR%" == "x" (
  set  "SWARM_CONFIG_DIR=%UNQUOTED_SWARM_HOME%\conf"
)

echo ===============================================================================
echo.
echo   Swarm startup Environment
echo.
echo   SWARM_HOME: %UNQUOTED_SWARM_HOME%
echo.
echo   JAVA: %JAVA%
echo.
echo   JAVA_OPTS: %JAVA_OPTS%
echo.
echo   CONFIG_DIR: %SWARM_CONFIG_DIR%
echo.
echo ===============================================================================
echo.
...
:RESTART
"%JAVA%" %JAVA_OPTS% ^
    -Dswarm.home.dir="%UNQUOTED_SWARM_HOME%" ^
